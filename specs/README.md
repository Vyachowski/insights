...
- **Response Wrapper**: every successful response is `{ data: T }`; every error is `{ error: { code, message, details? } }` — both typed in `contracts/api.types.ts`.
...
- **Components dispatch, never fetch.** Components and hooks must trigger data loading via `dispatch(someThunk())`, never by calling the `api/` layer directly (e.g. no `sitesApi.fetchAll()` inside a `useEffect`). The `api/` layer is reachable **only** from `store/thunks/`. Calling it from a component bypasses the slice, so the result lands in local `useState` instead of the store — unshareable, uncached, and outside the standard loading/error handling. If a domain has no thunk/slice yet, add one rather than reaching into `api/`.
...
- **Contracts**: Shared types in `contracts/` should be validated against Zod schemas generated by Prisma.
...
1. **Define Contract**: Add new DTO and API interface types to `contracts/`.
