  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h2 className="text-white font-semibold text-lg">Звонки</h2>
        ...
        <div className="flex items-center gap-3">
          ...
          <CsvImportButton onImport={callsApi.importCsv} onSuccess={load} label="Импорт CSV" />
        </div>
      </div>
              ...
              <table className="w-full">
                <thead className="sticky top-0 z-10 bg-slate-900 border-b border-slate-700/50">
                  <tr>
                    ...
                    <th className="text-left px-6 py-3.5 text-xs font-medium text-slate-400 uppercase tracking-wider">Откуда</th>
                    <th className="text-left px-6 py-3.5 text-xs font-medium text-slate-400 uppercase tracking-wider">Город</th>
                    <th className="text-left px-6 py-3.5 text-xs font-medium text-slate-400 uppercase tracking-wider">Канал</th>
                    <th className="text-right px-6 py-3.5 text-xs font-medium text-slate-400 uppercase tracking-wider whitespace-nowrap">№</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-700/30">
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan={5} className="py-16 text-center text-slate-500 text-sm">Нет звонков за {selectedYear} год</td>
                    </tr>
                  ) : (
                    pageEntries.map(entry => (
                      <tr key={entry.id} className="hover:bg-slate-800/30 transition-colors">
                        <td className="px-6 py-4 text-slate-300 text-sm whitespace-nowrap">{formatDate(entry.date)}</td>
                        <td className="px-6 py-4 text-slate-300 text-sm font-mono">{entry.src}</td>
                        <td className="px-6 py-4 text-sm">
                          <span className="text-xs px-2 py-1 rounded-md bg-slate-800 text-slate-300 capitalize">{entry.projectTitle}</span>
                        </td>
                        <td className="px-6 py-4 text-slate-400 text-sm">{entry.advChannelName || '—'}</td>
                        <td className="px-6 py-4 text-right text-slate-400 text-sm tabular-nums">{entry.callNumber}</td>
                      </tr>
                    ))
                  )}
                </tbody>
