...
export default function MetricsTabView({ loading, error, entries, sites, effectiveYear, availableYears, onYearChange, onImportClick }: Props) {
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-slate-700/30">
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan={6} className="py-16 text-center text-slate-500 text-sm">Нет метрик за {effectiveYear} год</td>
                    </tr>
                  ) : (
                    pageEntries.map(entry => (
                      <tr key={entry.id} className="hover:bg-slate-800/30 transition-colors">
                        <td className="px-6 py-4 text-slate-300 text-sm whitespace-nowrap">{formatDate(entry.date)}</td>
                        <td className="px-6 py-4 text-slate-300 text-sm">{getSiteName(entry.siteId, sites)}</td>
