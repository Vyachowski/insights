...
## Purpose
The cross-cutting HTTP contract for the backend API: versioned base route, cookie-based JWT auth, uniform `{ data }` / `{ error }` response envelopes, and the health check.
