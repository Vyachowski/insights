...
## Purpose
How the single React Router 7 framework-mode application is organized: routes with loaders/actions as the only data paths, server-only modules (Drizzle db/schema, cookie-session auth, bootstrap, CSV imports), route error boundaries, and the health endpoint.
...
Database access, auth, bootstrap, env validation, and CSV import logic SHALL live in server-only modules (`app/server/`), never imported into client components. The database schema SHALL be a single Drizzle TS file mirroring the existing table/column names; `drizzle-kit` SHALL own migrations.
