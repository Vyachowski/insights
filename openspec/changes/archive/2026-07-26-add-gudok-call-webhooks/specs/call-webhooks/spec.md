...
Receiving Gudok call-completion webhooks and collecting every call into the `calls` table: a secret-gated public endpoint, mapping of the Gudok payload to the schema, best-effort resolution of the owning site, retention of the raw payload, and idempotent storage keyed on the Gudok call id.
