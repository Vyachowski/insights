...
- [ ] 1.1 Scaffold RR7 framework-mode app at `apps/web` (Vite, TS, vitest) as a workspace alongside the existing apps; explicit `routes.ts`; root error boundary
...
- [ ] 2.1 Drizzle schema (`app/server/schema.ts`) mirroring current tables/columns 1:1 (snake_case names, enums, indexes, partial unique indexes from the old second migration)
- [ ] 2.2 `app/server/db.ts` (node-postgres pool + drizzle client); `drizzle-kit generate` init migration; programmatic `migrate()` in server entry
...
- [ ] 4.2 Port `DashboardPage` components/widgets/skeletons; loader with date/site filters via URL search params; delete the useEffect-fetch pattern
- [ ] 4.3 Verify recharts under SSR/hydration; apply hydrate-only rendering if needed; visual parity check against old app side-by-side
...
- [ ] 6.1 Delete `apps/backend`, `apps/frontend`, `contracts/`; flatten `apps/web` to repo root; remove npm workspaces; single root package.json
...
- [ ] 7.1 Update Railway service: build/start commands for the RR7 app; remove `JWT_EXPIRATION` var; keep the rest
