...
- [x] 3.1 Delete the two existing migration folders and generate a single fresh `init` migration from the current `schema.prisma` — `20260705092230_init`
- [x] 3.2 Recreate local dev DB: `dev:up` → `prisma migrate deploy` → verify schema matches — `migrate status` clean
