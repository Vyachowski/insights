...
- [x] 1.1 Add repo-root `nixpacks.toml` with a build phase: `cmds = ["npm run build", "npm prune --omit=dev"]`
...
- [ ] 3.2 Watch the Railway build log for the `npm prune --omit=dev` step and a clean server boot
